...
<div class="flex justify-between items-center mb-8">
    ...
    <div class="flex items-center gap-3">
        ...
        <span class="px-4 py-1 bg-[#832A2A] text-white text-sm rounded-full font-medium">Admin</span>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-md border border-[#BABABA] overflow-hidden">
    <div class="flex justify-between items-center px-6 py-5 border-b border-[#BABABA]">
        ...
        <div class="flex gap-3">
            {{-- Tombol Baru ke Halaman Category --}}
            <a href="{{ route('admin.categories.index') }}" class="bg-white border-2 border-[#CD2828] text-[#CD2828] px-5 py-2 rounded-lg font-bold hover:bg-red-50 transition-all flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/></svg>
                Manage Categories
            </a>
            
            <a href="{{ route('admin.products.create') }}" class="bg-[#1BCFD5] text-white px-5 py-2 rounded-lg font-medium hover:opacity-90 transition-colors flex items-center gap-2 shadow-sm">
            ...
            <tbody class="divide-y divide-[#BABABA]">
                @forelse($products as $p)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm">
                            <a href="{{ route('admin.products.show', $p) }}" class="text[#BABABA] hover:underline">Show</a>
                            ...
                            <form action="{{ route('admin.products.destroy', $p->id) }}" method="POST" class="inline">
                                ...
                                @method('DELETE')
                                <button type="submit" 
                                        onclick="return confirm('Yakin mau hapus {{ $p->name }}?')"
                                        class="text-[#CD2828] hover:underline">
                                    Delete
                                </button>
                            </form>
                        </td>
