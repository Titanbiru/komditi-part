...
<div class="py-4">
    ...
    <div class="bg-white border-2 border-gray-50 rounded-[2.5rem] p-8 shadow-xl shadow-gray-100/50">
        ...
        {{-- Header dengan Tombol Back yang Lebih 'Pop' --}}
        ...
        <form action="{{ route('user.account.addresses.update', $address->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                    ...
                    @foreach(['Rumah', 'Kantor', 'Apartemen', 'Kos'] as $label)
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="{{ $label }}" class="hidden peer" 
                                {{ (old('type', $address->type ?? 'Rumah') == $label) ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="recipient_name" value="{{ old('recipient_name', $address->recipient_name) }}" 
                        class="w-full bg-gray-50/50 border-2 border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-bold focus:outline-none focus:border-cyan-400 focus:bg-white transition-all">
            ...
            <div class="space-y-2">
                ...
                <textarea name="address" rows="3" 
                    class="w-full bg-gray-50/50 border-2 border-gray-100 rounded-[1.5rem] px-6 py-4 text-sm font-bold focus:outline-none focus:border-cyan-400 focus:bg-white transition-all resize-none">{{ old('address', $address->address) }}</textarea>
            </div>
