...
<div class="bg-white border border-gray-100 rounded-[2.5rem] p-4 shadow-sm">
    <div class="flex justify-between items-center mb-8">
        <h3 class="text-sm font-black text-gray-800 uppercase tracking-widest text-left">Alamat Kamu</h3>
    ...
    @if($addresses->isEmpty())
        <div class="py-20 text-center space-y-4">
            ...
            <p class="text-gray-500 font-bold uppercase text-xs">Kamu belum ada alamat untuk mengirim</p>
        </div>
    @else
        <div class="space-y-4">
            @foreach($addresses as $address)
                <div class="border border-gray-100 p-6 rounded-3xl flex justify-between items-start gap-4 {{ $address->is_default ? 'bg-cyan-50/30 border-cyan-100' : '' }}">
                    <div class="flex gap-6">
                        <span class="text-[10px] font-black uppercase text-cyan-400 bg-cyan-50 px-3 py-1 rounded-full h-fit">
                            {{ $address->type }}
                        </span>
                        <div class="space-y-1">
